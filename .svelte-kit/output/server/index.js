...
async function render_page(event, route, options, state, resolve_opts) {
  ...
  try {
    ...
    const should_prerender = ((_a = leaf_node.shared) == null ? void 0 : _a.prerender) ?? options.prerender.default;
    if (should_prerender) {
  ...
  const event = {
    ...
    setHeaders: (new_headers) => {
      for (const key2 in new_headers) {
        ...
        headers[lower] = new_headers[key2];
